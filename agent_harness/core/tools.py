...
DEFAULT_TRUSTED_DATA_SCOPES = frozenset({"internal"})
TOOL_EXECUTION_MANIFEST_SCHEMA = (
    "agent_harness.tool_execution_manifest.v1"
)
...
def _noop_effect_boundary() -> None:
    """Allow direct handler tests while the executor supplies durability."""
...
class ToolSpec:
    ...
    def validated(self) -> "ToolSpec":
        ...
        if (
            self.replay_policy != "safe"
            and self.execution_isolation != "trusted_inline"
        ):
            raise HarnessContractError(
                "non-safe tools require a trusted inline two-phase effect boundary"
            )
...
class ToolExecutionContext:
    ...
    begin_effect: Callable[[], None] = field(
        default=_noop_effect_boundary,
        repr=False,
        compare=False,
    )
...
def execute_tool_call(
    ...
    *,
    ...
    def begin_effect() -> None:
        ...
        if spec.replay_policy == "safe":
            raise ToolExecutionError(
                "safe tool attempted to cross an external-effect boundary",
                code="unexpected_effect_boundary",
            )
        if effect_started:
            return
        emitter.emit(
        ...
        # Set the local fact immediately after the durable event. If checkpoint
        # persistence fails, the handler still has not been allowed to perform
        # its effect and recovery conservatively sees the journal boundary.
        effect_started = True
    ...
    for attempt in range(1, spec.retry_policy.max_attempts + 1):
        ...
        try:
            ...
            if spec.replay_policy != "safe" and not effect_started:
                # A non-safe handler which did not explicitly declare the
                # commit boundary may already have changed the outside world.
                # Fence it as uncertain instead of recording a false success.
                begin_effect()
                raise ToolExecutionError(
                    "non-safe tool did not declare its effect boundary",
                    code="effect_boundary_missing",
                )
            ...
            if len(rendered) > max_output_chars:
                ...
                # The external effect has already happened. Reporting failure
                # here would invite the model to repeat a never-replay or
                # idempotent action. Settle it as completed with a bounded,
                # digest-backed projection instead.
                original_result_hash = canonical_sha256(result)
                result = {
                    "truncated": True,
                    "result_omitted": "tool output exceeded the display budget",
                    "original_result_sha256": original_result_hash,
                }
                rendered = canonical_json(result)
