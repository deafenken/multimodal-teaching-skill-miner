...
The journal is deliberately independent from application persistence. It is
...
HARNESS_JOURNAL_RECORD_SCHEMA = "agent_harness.journal_record.v1"
HARNESS_JOURNAL_CHECKPOINT_SCHEMA = "agent_harness.journal_checkpoint.v1"
...
def _prepare_parent(path: Path, *, label: str) -> None:
    ...
    if stat.S_ISLNK(parent_stat.st_mode) or not stat.S_ISDIR(parent_stat.st_mode):
        raise HarnessJournalError(f"{label} parent must be a real directory")
    if parent_stat.st_mode & 0o077:
        if created:
            os.chmod(parent, 0o700)
            return
        raise HarnessJournalError(f"{label} parent permissions are too broad")
...
def _reject_non_regular_target(path: Path, *, label: str) -> bool:
    ...
    if target_stat.st_mode & 0o077:
        raise HarnessJournalError(f"{label} permissions are too broad")
    return True
...
class HarnessJournal:
    ...
    def _open_journal(self) -> Iterator[int]:
        ...
        try:
            ...
            if not stat.S_ISREG(file_stat.st_mode) or file_stat.st_mode & 0o077:
                raise HarnessJournalError("journal path must be a private regular file")
    ...
    def _read_checkpoint_envelope(self) -> dict[str, Any] | None:
        ...
        try:
            ...
            if not stat.S_ISREG(file_stat.st_mode) or file_stat.st_mode & 0o077:
                raise HarnessJournalError(
                    "checkpoint path must be a private regular file"
                )
            data = _read_all(file_descriptor)
        finally:
            os.close(file_descriptor)
...
# implementation or a dependency on an application persistence layer.
