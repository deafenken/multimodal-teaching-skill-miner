...
explicitly opt in before user text can leave the machine.
"""
...
def _read_api_key(*, api_key: str | None, api_key_file: str | Path | None) -> str:
    ...
    configured_file = (
        api_key_file
        or os.getenv("HARNESS_DEEPSEEK_API_KEY_FILE")
    )
    ...
    path = Path(configured_file).expanduser().absolute()
    try:
        ...
        if (
            ...
            raise DeepSeekConfigurationError(
                "DeepSeek API key directory must be private and non-symlinked"
            )
        ...
        if stat.S_ISLNK(entry_stat.st_mode):
            ...
            if (
                stat.S_ISLNK(target_parent.st_mode)
                or not stat.S_ISDIR(target_parent.st_mode)
                or target_parent.st_mode & 0o077
            ):
                raise DeepSeekConfigurationError(
                    "DeepSeek API key target directory must be private"
                )
        else:
            target = path
        ...
        if (
            ...
            or not 8 <= target_stat.st_size <= 4_096
        ):
            raise DeepSeekConfigurationError(
                "DeepSeek API key file must be a private regular file"
            )
        flags = os.O_RDONLY
        ...
        try:
            ...
            if (
                ...
                or opened.st_ino != target_stat.st_ino
                or opened.st_size != target_stat.st_size
            ):
                raise DeepSeekConfigurationError(
                    "DeepSeek API key file changed while opening"
                )
            raw = os.read(descriptor, 4_097)
        finally:
            os.close(descriptor)
        if len(raw) > 4_096:
            raise DeepSeekConfigurationError("DeepSeek API key file is too large")
        key = raw.decode("utf-8").strip()
    except DeepSeekConfigurationError:
        raise
    except UnicodeDecodeError as exc:
        raise DeepSeekConfigurationError(
            "DeepSeek API key file is not UTF-8"
        ) from exc
...
class DeepSeekConfig:
    ...
    def from_environment(
        ...
        *,
        api_key_file: str | Path | None = None,
        allow_remote_content: bool | None = None,
        model: str | None = None,
    ) -> "DeepSeekConfig":
        timeout = float(os.getenv("HARNESS_DEEPSEEK_TIMEOUT_SECONDS", "60"))
        retries = int(os.getenv("HARNESS_DEEPSEEK_MAX_RETRIES", "2"))
        temperature = float(os.getenv("HARNESS_DEEPSEEK_TEMPERATURE", "0"))
        return cls(
            base_url=os.getenv("HARNESS_DEEPSEEK_BASE_URL", DEFAULT_BASE_URL),
            model=model or os.getenv("HARNESS_DEEPSEEK_MODEL", DEFAULT_MODEL),
            timeout_seconds=timeout,
            max_retries=retries,
            thinking_enabled=_truthy(os.getenv("HARNESS_DEEPSEEK_THINKING")),
            temperature=temperature,
            allow_remote_content=(
                _truthy(os.getenv("HARNESS_ALLOW_REMOTE_CONTENT"))
                if allow_remote_content is None
                else allow_remote_content
            ),
    ...
    def validated(self) -> "DeepSeekConfig":
        if not isinstance(self.base_url, str) or not isinstance(self.model, str):
            raise DeepSeekConfigurationError("DeepSeek endpoint and model must be strings")
        if type(self.thinking_enabled) is not bool or type(self.allow_remote_content) is not bool:
            raise DeepSeekConfigurationError("DeepSeek boolean settings must be exact booleans")
        if type(self.max_retries) is not int or type(self.max_tokens) is not int:
            raise DeepSeekConfigurationError("DeepSeek integer settings are invalid")
        if (
            ...
            raise DeepSeekConfigurationError("DeepSeek numeric settings are invalid")
    ...
    def public_status(self) -> dict[str, Any]:
        return {
            ...
            "remote_content_opt_in": self.allow_remote_content,
...
class DeepSeekClient:
    ...
    def probe_model_availability(
        ...
        The official ``GET /models`` endpoint receives no user content and
        ...
        headers = {
            ...
            "User-Agent": "AgentHarness-Readiness/1.0",
        }
        ...
        if self.config.model not in models:
            raise DeepSeekClientError("DeepSeek readiness probe failed")
        return {
            "schema": "agent_harness.deepseek_readiness.v1",
    ...
    def chat_web(
        ...
        *,
        ...
        if require_remote_consent and not self.config.allow_remote_content:
            raise DeepSeekConfigurationError(
                "remote user-text processing is disabled; pass explicit consent"
            )
    ...
    def chat_json_stream(
        ...
        *,
        ...
        assistant deltas: they contain the planner envelope, not user-facing
