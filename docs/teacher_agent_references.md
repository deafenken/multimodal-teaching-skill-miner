...
自动模式让 DeepSeek 对会话内允许的 Skill 提出排序，再由控制器按本轮信号、角色、纠错证据与重复上限执行硬校验；首轮因没有回答，会先过滤为支持 `not_observed` 的候选。手动 `/+skill 名称` 是持续锁定而不是一次性提示：它从下一条学生回答开始生效，直到 `/auto` 或安全门释放，并且不能绕过适用条件、纠错证据、`max_repeat`、fallback 与终止规则。
