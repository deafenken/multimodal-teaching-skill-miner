...
> 我们实现的是一个真正逐轮运行的混合 Teaching Agent。DeepSeek V4 Flash 负责理解学生自然语言、判断当前问题对齐、建议 Skill 并生成当前动作候选；确定性控制器维护状态与安全边界。通过最终 Skill、单动作、问题契约和防答案泄露门禁的安全话语可以展示，否则由确定性 materializer 接管。系统默认使用 v2 的 13 个主 Skill 与 3 个支持 Skill，并能显示每轮选择、组合、切换、理由和动作来源。28 例作者构造单轮开发集有已记录数值；另有 20 episode、65 个学生回答回合并含 1 次画像替换操作的作者构造多轮评估管线。最新私有 v2 run19 的分维度结果仍只作 development regression，不替代专家锁箱、真实学生学习效果和部署验证；lifecycle receipt 仅为结构与哈希自校验，未获外部签名。
