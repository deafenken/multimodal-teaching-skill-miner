...
| CI 与隔离 wheel smoke | 当前本地发布验收通过；远端 CI 独立核验 | 当前本地全仓测试为 **661 passed、1,234 subtests、0 skipped**，`build_release_acceptance.sh` 已完成双次洁净构建、字节一致性、exact-wheel 安装/入口 smoke、发布审计与机器收据；任何后续 tracked-file 修改都会使该收据失效并要求重生成。`.github/workflows/ci.yml` 的 GitHub Actions run 30209305856 只证明已推送 HEAD `13e12c172ae3…` 通过 Python 3.10–3.13、package/privacy 与 acceptance；当前分支的新提交仍须以其自己的 GitHub run 结果为准。`remote_github_actions_run_verified=false` 表示远端状态不进入本地机器证明链 |
