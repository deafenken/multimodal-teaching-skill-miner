...
| command bar 与 recovery console | 完成工程/UI 验收 | `GOAL / PLAN / PROGRESS / CONTROL`、`/auto`、`/+skill`、`/stop`、可恢复 `cancel_turn`、草稿保留重试与转人工已接入；stale guard、幂等键和 commit fence 防止旧响应覆盖新会话。浏览器证据仅覆盖受控 Chrome 路径。 |
...
题目二当前工程契约为 V15（`teaching_agent_assess_route_act_v15_correction_chain_taxonomy_contract`）。`action_only_repair` 只允许固定路由下修复当前动作，`continuity_recall` 必须绑定历史证据；纠错链的 canonical taxonomy 与 teacher-claim contract 只允许在唯一目标、高置信、学生原话证据满足时参与归一化；这些字段和 `cancel_turn` 的边界以生产 dashboard 实现为准。
