...
| CI 与隔离 wheel smoke | 当前本地发布验收通过；远端 CI 独立核验 | 直接本地全量测试为 **791 passed、1,354 subtests**；同一 `build_release_acceptance.sh` 收据记录 791 个测试用例（786 passed、5 个仅因 sandbox 禁止 loopback socket 的环境跳过），并通过双次洁净构建、字节一致性、exact-wheel 安装/入口 smoke、发布审计与机器收据。任何后续 tracked-file 修改都会使该收据失效并要求重生成。GitHub Actions 只作为推送提交的独立远端检查，当前分支的新提交须以其自己的 CI run 为准；`remote_github_actions_run_verified=false` 表示远端状态不进入本地机器证明链 |
