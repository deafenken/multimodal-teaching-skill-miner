...
| Headless | `claude -p`、stream-json、budget、Agent SDK。[官方 headless 文档](https://code.claude.com/docs/en/headless) | `codex exec`、JSONL、resume 与 SDK。[非交互模式](https://developers.openai.com/codex/noninteractive)、[SDK](https://developers.openai.com/codex/sdk) | 已实现 `harness exec`、`--jsonl`、稳定退出码和 session 续接；成本 budget 尚未实现。 |
| SDK | Claude Agent SDK 是 Python/TypeScript 库，提供 sessions、permissions、tools、hooks、subagents、MCP 和流式输出。[官方概览](https://code.claude.com/docs/en/agent-sdk/overview) | 官方 TypeScript SDK 要求 Node.js 18+，可 start/continue/resume 本地 thread；稳定 Python SDK 要求 Python 3.10+，通过本地 app-server JSON-RPC 并携带 pinned runtime，提供同步/异步接口和 sandbox preset。[官方 SDK](https://developers.openai.com/codex/sdk) | 已实现两个边界不同的稳定接口：Python 是 `AgentRunner` 进程内 sync/async SDK，支持 start/resume/fork、run/stream、附件、取消和显式 `ApprovalBroker`；TypeScript 是 Node.js 18+、零运行时依赖的 `harness exec --jsonl` 子进程 SDK，支持 lazy start/resume、run/stream、附件、每轮权限和 `AbortSignal`。TS 使用 `shell: false` 与有界严格 JSONL 校验，但无 fork/approval broker，且不是 app-server transport；不声称 Claude/Codex SDK 全能力 parity。 |
...
- 稳定 SDK 面：Python 进程内 sync/async start/resume/fork、run/stream、附件、取消和显式
  approval broker；TypeScript Node.js 18+ dependency-free subprocess/JSONL lazy start/resume、
  run/stream、附件、逐轮权限、`AbortSignal`、有界协议校验和安全 stderr 处理。

## P1 顺序

1. 在不削弱 exact trust、冻结 catalog、once-only approval 和 sandbox 边界的前提下，分阶段
...
2. 扩展 hooks 生命周期与异步模式，但只有在不削弱 exact trust、单调权限和沙箱边界时推进。
3. 单独 threat model 后再评估后台/可恢复 agent threads、自定义 agents、模型路由、显式
