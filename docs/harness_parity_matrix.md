...
1. provider-neutral text/PDF/image attachment contract。
2. 稳定 Python/TypeScript SDK。
3. 在不削弱 exact trust、冻结 catalog、once-only approval 和 sandbox 边界的前提下，分阶段
...
4. 扩展 hooks 生命周期与异步模式，但只有在不削弱 exact trust、单调权限和沙箱边界时推进。
5. 单独 threat model 后再评估后台/可恢复 agent threads、自定义 agents、模型路由、显式
