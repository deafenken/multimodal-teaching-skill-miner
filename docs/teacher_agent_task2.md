...
| 历史对话 | 按知识点、关注维度和最近轮次选择相关历史，较早历史压缩为统计摘要 | 已实现；不是无限上下文 |
...
flowchart LR
    ...
    H["受限历史管理<br/>同知识点 → 同关注点 → 最近轮次<br/>最多 6 轮 / 8000 字符"] --> C
...
`deepseek-v4-flash` 每轮接收经过最小化处理的目标、非身份画像字段、显式学生状态、当前 Goal 步骤、当前动作、相关历史、学生回答和 Skill 摘要，返回严格 JSON：
...
只有经过约束层验证的 DeepSeek 诊断才产生候选；初始动作和 `deterministic_safety_fallback` 都不会生成。候选证据在保存前已做常见直接标识符模式替换，并必须是本轮脱敏回答的真实子串；模型伪造摘录时会回退到本轮脱敏文本。完整候选画像不会作为下一次远程请求中的画像字段再次发送。
...
远程请求不会无界发送整个会话。相关历史按“同知识点优先、同关注维度其次、最近轮次最后”选择，默认最多 6 轮、序列化上限 8000 字符；更早轮次只保留信号、重点和 Skill 的聚合计数。
...
2. 支持 Skill 去重后最多保留 2 个；
3. 当前 Skill 不能超过自己的 `max_repeat`；
4. 输出必须只有一个可响应动作，并明确等待学生；
5. 每轮记录上一 Skill、是否切换、选择理由、模型 trace 和状态证据；
6. 低置信度且建议人工复核时，状态更新会保守按 `confused` 处理；
7. 模型建议停止只有同时满足“建议人工复核 + 至少两轮无进展”才会被控制器采纳。
...
页面只绑定 `127.0.0.1`，使用随机 capability URL、CSP 和 `Cache-Control: no-store`，单个会话只保存在服务内存。页面本地运行不等于全部处理离线：在线模式会把最小化并做常见直接标识符模式替换后的文本发送给配置的 DeepSeek API。
